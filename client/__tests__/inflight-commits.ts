...
describe(InflightCommit, () => {
  ...
  describe(InflightCommit.randomRef, () => {
    test("randomRef, with node crypto", () => {
      expect(typeof InflightCommit.randomRef()).toBe("string");
      expect(InflightCommit.randomRef()).not.toEqual(
        InflightCommit.randomRef()
      );
    });
    test("randomRef, with window crypto", () => {
      global.window = {
        crypto: {
